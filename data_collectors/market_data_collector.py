...
class MarketDataCollector:
    def __init__(self, db_manager: DatabaseManager):
        ...
        # 从配置文件读取并发数
        self.concurrency_limit = config.get('network', {}).get('concurrency_limit', 3)
        logging.info(f"初始化行情数据收集器，并发数: {self.concurrency_limit}")
    ...
    async def fetch_and_store_kline_data(self, start_date: str = None, end_date: str = None):
        ...
        try:
            ...
            semaphore = asyncio.Semaphore(self.concurrency_limit)  # 使用配置文件中的并发数
    ...
    async def _fetch_and_store_single_stock_data(self, stock_code, start_date, end_date, index, total_count):
        ...
        while retry_count <= max_retries:
            try:
                logging.info(f"[{index + 1}/{total_count}] 开始获取股票 {stock_code} 的日、周、月K线数据 (尝试 {retry_count + 1}/{max_retries + 1})")
                ...
                if retry_count <= max_retries:
                    ...
                    logging.warning(f"获取股票 {stock_code} 数据时出错: {e}，{wait_time:.2f}秒后进行第 {retry_count + 1} 次重试")
                    await asyncio.sleep(wait_time)
                else:
                    logging.error(f"获取股票 {stock_code} 行情数据时出错（已重试{max_retries}次）: {e}", exc_info=True)
                    raise
